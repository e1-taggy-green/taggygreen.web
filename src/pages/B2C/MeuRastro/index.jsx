    ...
    return (
      <div className="bg-white border border-gray-200 rounded-xl px-3 py-2 shadow-md text-sm">
        ...
        <p className="text-green-600 font-black">
          {payload[0].value.toFixed(1).replace(".", ",")} kg CO₂
        </p>
      </div>
    );
  }
...
/** Conversor de Impacto — conforme especificação:
 *  - Árvores plantadas
 *  - Combustível economizado
 *  - Horas LED
 */
function ConversorImpacto({ arvores, combustivel, horasLed }) {
  return (
    <div className="grid grid-cols-3 gap-4">
      <div className="bg-green-50 rounded-2xl p-4 flex flex-col items-center text-center gap-1">
        ...
        <span className="text-xs text-gray-400">equivalentes/ano</span>
      </div>
      ...
      <div className="bg-blue-50 rounded-2xl p-4 flex flex-col items-center text-center gap-1">
        ...
        <span className="text-xs text-gray-400">litros</span>
      </div>

      <div className="bg-amber-50 rounded-2xl p-4 flex flex-col items-center text-center gap-1">
        <div className="w-10 h-10 bg-white bg-opacity-60 rounded-xl flex items-center justify-center mb-1">
          <Lightbulb size={22} className="text-amber-500" />
        </div>
        <span className="text-xs text-gray-500 font-semibold">Horas LED</span>
        <span
          className="text-2xl font-black text-gray-900"
          style={{ fontFamily: "'Syne',sans-serif" }}
        >
        ...
        <span className="text-xs text-gray-400">horas de iluminação</span>
      </div>
    </div>
  );
}

/** Item do histórico — sem localização, apenas Pedágio ou Estacionamento */
function ExtratoItem({ item }) {
  const isPedagio = item.tipo === "pedagio" || !item.tipo || item.praca?.toLowerCase().includes("pedágio");
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            >
              {isPedagio ? "Pedágio" : "Estacionamento"}
            </span>
            <span className="text-xs text-gray-400">
              {item.data} · Tag {item.tag}
            </span>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-14 text-center px-6">
      ...
      <p className="font-bold text-gray-800 text-base">
        Você ainda não passou por pedágios.
      </p>
      <p className="text-sm text-gray-400 mt-1">
        Sua jornada verde está prestes a começar!
      </p>
    </div>
  );
}
...
/**
 * B2C — Meu Rastro Verde
 * Revisado conforme especificação:
 * - Conversor de Impacto: Árvores plantadas / Combustível economizado / Horas LED
 * - Histórico: sem localização, apenas Pedágio ou Estacionamento
 * - Gráfico de evolução presente
 * - Dados via backend (useRastroVerde)
 */
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 bg-gray-50 px-6 py-7">
        <div className="max-w-6xl mx-auto space-y-5">
            ...
            <div className="relative bg-gradient-to-br from-green-700 to-green-950 rounded-3xl p-8 flex items-center justify-between overflow-hidden shadow-xl">
              ...
              <div className="relative z-10">
                ...
                <p className="text-white/65 text-sm max-w-xs leading-relaxed">
                  de CO₂ mitigados com suas passagens pela Tag Edenred nos
                  pedágios.
                </p>
              </div>
          ...
          {/* Conversor de Impacto: Árvores / Combustível / Horas LED */}
          {loading ? (
            <div className="grid grid-cols-3 gap-4">
              {[1, 2, 3].map((i) => (
                  ...
                  <Bar dataKey="kg" radius={[8, 8, 0, 0]}>
                    {historico.map((_, i) => (
                      <Cell
                        key={i}
                        fill={
                          i === historico.length - 1 ? "#16a34a" : "#86efac"
                        }
                      />
                    ))}
                  </Bar>
            ...
            <div className="bg-green-50 border border-green-200 border-l-4 border-l-green-500 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-green-700 leading-relaxed">
                  Todos os cálculos estão em conformidade com as diretrizes do
                  GHG Protocol para emissões de escopo 1 e 3.
                </p>
              </div>
            </div>
          )}
